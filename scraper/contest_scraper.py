...
# starts the selenium driver to handle exceptions
driver = sl.site_login_driver()

# find and save the projects [function is way to big, sorry for that]
def find_projects(status, winorloose, contest_page):
    ...
    for i in status.find_all('div', class_='card-body'):
        try:
            ...
            print("Whole Scraper didn´t worked")
...
def project_classification(contest_page):
    ...
    while i <= 12:
        try:
            ...
            else: # the following pages just contain 'loosers'
